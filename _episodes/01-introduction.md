...
## Step 2: Download and install UB's Cisco VPN client. You will be receiving a separate email from Globus with the link to download this software. 
...
When you start the Cisco software the first time you will need to enter the following in the box labeled
"Connect to:" `vpn.buffalo.edu` and then select CCR from the group drop down menu. Enter the UB VPN username WITHOUT the itorg in front (e.g. `uccr.UBID` - remember
to substitute UBID with you actual username) and password provided. Then you'll be prompted to ask how you want to received the second factor from Duo.

Now that you have setup the UB account and connected to the VPN, you'll be able to move forward with your CCR account.


## Step 4:  Make sure you're connected to the UB VPN and go to CCR's identity management (IDM) portal, enter your CCR username (e.g. UBID) and click the Next button. 
 ...
 * OnDemand web portal:  [https://ondemand.ccr.buffalo.edu](https://ondemand.ccr.buffalo.edu)

 * Via a SSH client, you may login to our pool of front end servers with the address: `login.ccr.buffalo.edu`.
   You must use SSH keys as we do not accept passwords over SSH. Please follow our instructions here to enable SSH access.
